  ...
  return (
    <div className="relative flex flex-col h-full overflow-hidden">
      ...
      <div
        ...
        style={{ background: 'radial-gradient(circle, #F7C5D8, transparent 70%)', animation: 'float 4s ease-in-out infinite' }}
      />
      <div
        className="absolute -bottom-10 right-0 w-48 h-48 rounded-full opacity-40"
        style={{ background: 'radial-gradient(circle, #DDD5F7, transparent 70%)', animation: 'float 5.5s ease-in-out infinite 1s' }}
      />
...
function PasswordStrength({ password }: { password: string }) {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <p className={`text-[11px] mt-1 font-semibold ${score >= 3 ? 'text-aura-mint-dark' : 'text-gray-400'}`}>
        {labels[score]}
      </p>
    </div>
  )
