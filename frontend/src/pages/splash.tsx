  ...
  return (
    <div className="relative flex flex-col items-center justify-between h-full px-8 pt-16 pb-12 overflow-hidden">
      ...
      <div
        ...
        style={{ background: 'radial-gradient(circle, #F7C5D8, transparent 70%)' }}
      />
      <div
      ...
      <div className="relative z-10 text-center animate-fade-up animation-fill-both">
        <div className="inline-flex items-center gap-2 bg-white/50 backdrop-blur-sm border border-white/60 rounded-full px-4 py-1.5 mb-5">
          ...
          <span className="text-xs font-semibold text-aura-pink-dark tracking-wide">Your AI Bestie</span>
        </div>
        <h1 className="font-display font-black text-4xl leading-tight mb-3">
          <span className="text-gradient-pink">How are you</span>
          <br />
          <span className="font-display font-black text-4xl text-aura-lav-dark">feeling today?</span>
        </h1>
        <p className="font-body text-sm text-gray-500 leading-relaxed max-w-xs mx-auto">
          Meet your AI companions — always here to listen, support, and cheer you on. 🌸
        </p>
      </div>

      {/* Character illustration */}
      <div
        className="relative z-10 w-52 h-64 animate-fade-in animation-fill-both animation-delay-200"
      >
        ...
        >
          <span className="text-xs font-semibold text-aura-pink-dark">😊 Feeling good!</span>
        </div>
        <div
          className="absolute -right-6 bottom-16 glass-card px-3 py-1.5 animate-float animation-delay-500"
          style={{ animationDuration: '4s' }}
        >
          <span className="text-xs font-semibold text-aura-lav-dark">💜 Luna is here</span>
        </div>
      </div>
